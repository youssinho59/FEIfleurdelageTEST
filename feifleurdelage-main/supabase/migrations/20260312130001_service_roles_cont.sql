-- ──────────────────────────────────────────────────────────────
-- Système de services et rôles par service (suite)
-- La valeur 'responsable' de l'enum est déjà committée par la migration précédente.
-- ──────────────────────────────────────────────────────────────

-- 1. Ajouter la colonne service à user_roles (nullable)
ALTER TABLE public.user_roles ADD COLUMN IF NOT EXISTS service TEXT;

-- 2. Ajouter le service aux FEI et Plaintes
...
-- 3. Fonction helper : retourne le service d'un responsable
...
-- 4. Suppression des anciennes politiques permissives
...
-- 6. Nouvelles politiques Plaintes ───────────────────────────
